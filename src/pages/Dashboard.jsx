...
export default function Dashboard() {
  ...
  const done = orders.filter((o) => o.status === OS_STATUS.CONCLUIDO).length;
  const totalOs = orders.length;
  const running = orders.filter((o) => o.status === OS_STATUS.EM_EXECUCAO).length;
  const pending = orders.filter((o) => o.status === OS_STATUS.PENDENTE).length;
  const efficiency = totalOs > 0 ? Math.round((done / totalOs) * 100) : 0;
  ...
  // Agrupamento por status de equipamento (PORT)
  const agg = {
    [EQUIP_STATUS.OPERACIONAL]: 0,
    [EQUIP_STATUS.EM_MANUTENCAO]: 0,
    [EQUIP_STATUS.ATENCAO]: 0,
    [EQUIP_STATUS.OFFLINE]: 0,
  };
  equipments.forEach((e) => {
    if (agg[e.status] !== undefined) agg[e.status]++;
  });
  ...
  const pieData = {
    ...
    datasets: [
      {
        data: [
          agg[EQUIP_STATUS.OPERACIONAL],
          agg[EQUIP_STATUS.EM_MANUTENCAO],
          agg[EQUIP_STATUS.ATENCAO],
          agg[EQUIP_STATUS.OFFLINE],
        ],
